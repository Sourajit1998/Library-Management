...
public class Return_check extends HttpServlet
{
	Connection cn;
	...
	public void init()
	{
		try
		{
	        ...
	        //ps2=cn.prepareStatement("select book_name,category,issue_date from issue_studs where name=? and id=? order by issue_date asc");
		}
		catch(Exception ee)
		{
			ee.printStackTrace();
		...
		if(hs!=null)
		{
			try
			{
				...
				if(rs.next())               //He has been issued book earlier
				{
					...
					RequestDispatcher rd=req.getRequestDispatcher("Return1");
					rd.include(req, res);
				}
